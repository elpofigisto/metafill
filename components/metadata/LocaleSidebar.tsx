...
import type { LocaleOverview } from "./metadataClient";
...
function badgeFor({ locale, isSelected, dirty, reviewed, info }: BadgeArgs): Badge {
  if (isSelected && dirty) {
...
type LocaleSidebarProps = MetadataEditor;

export function LocaleSidebar({ actions, computed, state }: LocaleSidebarProps) {
  const {
    bulkLocales,
    busyAction,
    dirty,
  ...
  const visibleLocales = useMemo<string[]>(() => {

...
function newApp(existingIds: string[]): AppConfig {
...
export default function AppsManager() {
  ...
  const [lookupBusy, setLookupBusy] = useState<string>("");
  ...
  function updateApp(appId: string, patch: Partial<AppConfig>) {
    setApps((currentApps) =>
  ...
  async function runLookup(app: AppConfig) {
    ...
    try {
      ...
      setError(toMessage(lookupError));
    } finally {
      setLookupBusy("");
    }
  }

  async function submitApps(event: React.FormEvent<HTMLFormElement>) {
              ...
              <div className="app-grid">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateApp(app.id, { platform: event.target.value as AppConfig["platform"] })
                    }
                  >

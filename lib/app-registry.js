...
function normalizeSettings(rawSettings) {
  ...
  if (legacyKey) {
    const legacyProvider = settings.aiProvider || "anthropic";
    aiApiKeys[legacyProvider] = aiApiKeys[legacyProvider] || legacyKey;
  }
  settings.aiApiKeys = aiApiKeys;
...
export async function loadSettings(rootDir = process.cwd()) {
  ...
  for (const candidate of AI_PROVIDERS) {
    resolved.aiApiKeys[candidate] =
      settings.aiApiKeys?.[candidate] || (process.env[AI_KEY_ENV[candidate]] ?? "").trim() || "";
  }
  // The active provider's key, kept for downstream translate code that reads a
  // single `aiApiKey`.
  resolved.aiApiKey = resolved.aiApiKeys[provider] || "";

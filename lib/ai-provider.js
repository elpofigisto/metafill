...
// API keys are ASCII; strip whitespace and any stray non-printable/Unicode
// characters (e.g. a smart quote or hidden symbol from a paste) that would
// otherwise break HTTP header encoding with a ByteString error.
...
export async function listModels({ provider, apiKey }) {
  ...
  const models = await lister(cleanKey);
  const byId = new Map();

...
import { METADATA_FILES, SOURCE_LOCALE } from "./app-store-metadata.js";
...
export const LOCALE_STOREFRONTS = {
  "en-US": "us",
  "en-GB": "gb",
  "en-AU": "au",
  "en-CA": "ca",
...
function appQueryParam(app, params) {
  if (app.appStoreId) {
...
async function lookupResultForCountry(app, country) {
...
export async function lookupPublicAppInfo({ bundleId, appStoreId, country = "us" }) {
  const params = new URLSearchParams({ country });
...
function metadataFromLookupResult(result) {
  return {
...
export async function fetchPublicAppStoreMetadata(app) {
...
export async function fetchPublicLocalizations(app, locales) {
  ...
  const byCountry = Object.fromEntries(fetched.map((entry) => [entry.country, entry.result]));
  ...
  const results = mapped.map((locale) => {
...
export async function writeLocaleMetadata(metadataRoot, locale, files) {
...
export async function writePublicAppStoreMetadata(metadataRoot, files) {

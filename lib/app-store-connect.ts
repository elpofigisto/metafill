...
export async function prepareFastlaneAuthArgs(
  creds: Credentials,
  { rootDir, tempDir }: { rootDir: string; tempDir: string },
): Promise<string[]> {
  const args: string[] = [];
  ...
  if (keyPath) {
    if (keyPath.toLowerCase().endsWith(".json")) {
      ...
      try {
        ...
        throw new Error(
          `Could not read App Store Connect .p8 key at ${resolvedKeyPath}: ${toMessage(error)}`,
        );
      }
